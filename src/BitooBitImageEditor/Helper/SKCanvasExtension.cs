...
namespace BitooBitImageEditor.Helper
{
    internal static class SKCanvasExtension
    {
        ...
        internal static SKTypeface GetSKTypefaceForEmoji()
        {
            SKTypeface typeFace;

            var assembly = IntrospectionExtensions.GetTypeInfo(typeof(ImageResourceExtension)).Assembly;
            using(Stream stream = assembly.GetManifestResourceStream("BitooBitImageEditor.Resources.EmojiOne.ttf"))
            {
            ...
            try
            {
                if (!string.IsNullOrWhiteSpace(text))
                {
                    using (SKPaint paint = new SKPaint())
                    {
                        ...
                        var a = new SKShaper(SKTypeface.Default);
                       

                        canvas.DrawShapedText(a, text, rect.Left, rect.Top, paint);
                    }
                }
            }
            catch (Exception ex) 
...
//                            SKTypeface typeface = SKFontManager.Default.MatchCharacter(numberChar);
//                            string fontName = typeface.FamilyName.ToLower();
//                            if (fontName.Contains("emoji") || fontName.Contains("a5##fallback"))
//                                typeface = GetSKTypefaceForEmoji();


//                            charPaint.Typeface = charsTypeface[i][j] = typeface;
...
//                float scale = minTextSize / paint.TextSize;
//                ////////////canvas.Scale(scale);
